  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe(
        "gateway/temperature/zen-e6614103e7698839",
        (err) => {
          if (err) {
    ...
    mqttClient.on("message", (topic, message) => {
      try {
        ...
        if (topic === "gateway/temperature/zen-e6614103e7698839") {
          const temperatureData = data.metrics.find(
            (metric: any) => metric.name === "temperature"
          );
          if (temperatureData) {
        ...
        if (topic === "gateway/humidity/zen-e6614103e7698839") {
          const humidityData = data.metrics.find(
            (metric: any) => metric.name === "humidity"
          );
          if (humidityData) {
